 ...
 *              una Red Neuronal Feed-Forward con algoritmos optimizados.
 *
...
/* =============================================================================
 * CONSTANTES DE CONFIGURACION DE LA RED
 * La topologia es 4 -> HIDDEN_SIZE -> 3 (Entrada, Oculta, Salida)
 * =============================================================================
 */
 ...
 * ---------------------------------------------------------------------------
 * Representa una capa con sus pesos sinapticos, sesgos y activaciones.
 * Dimensiones: [num_neurons x num_inputs] para pesos, [num_neurons] para sesgos.
 *
 * Almacenamiento contiguo: se usa un unico bloque malloc para la matriz de pesos,
 * y se accede mediante aritmetica de punteros pura (sin array[i][j]).
 * =============================================================================
 */
typedef struct {
    int num_neurons;    /* Numero de neuronas en esta capa    */
    int num_inputs;     /* Numero de entradas a esta capa     */
    ...
    double *z;          /* Suma ponderada antes de activacion [num_neurons] */
} Layer;
 ...
 * en tiempo de ejecucion mediante malloc().
 * =============================================================================
 */
typedef struct {
    int num_layers;     /* Numero total de capas (incluyendo entrada y salida) */
    Layer *layers;      /* Array dinamico de capas [num_layers]                */
    double learning_rate; /* Tasa de aprendizaje (configurable)               */
    int epochs;           /* Numero de epocas de entrenamiento (configurable) */
} FeedForwardNN;

/* =============================================================================
 * ESTRUCTURA: Dataset
 * ---------------------------------------------------------------------------
 * Almacena las muestras leidas del archivo flores-dataset.txt.
 * Se reserva memoria dinamicamente y se separa en entrenamiento (80%) y
 * prueba (20%) para validacion.
 * =============================================================================
 */
typedef struct {
...
/*
 * Crea y retorna una red neuronal con la topologia configurada.
 * Reserva toda la memoria dinamicamente con malloc/calloc.
...
/*
 * Funcion de activacion Sigmoide: sigma(x) = 1 / (1 + e^(-x))
 ...
 * Utiliza el Algoritmo de Strassen para multiplicaciones de matrices.
 * Complejidad teorica de Strassen: O(n^2.807) vs O(n^3) del naive.
 */
...
/*
 * Entrena la red con el dataset completo por el numero de epocas.
 ...
 * Tamano debe ser potencia de 2 (se redondea hacia arriba internamente).
...
/*
 * Multiplicacion naive de matrices para comparacion o fallback.
 * Complejidad: O(n^3) - algoritmo clasico triple bucle anidado.
 */
void naive_multiply(double *A, double *B, double *C, int n);

/*
 * Multiplica matriz [m x k] por vector [k] -> resultado [m].
 * Utiliza aritmetica de punteros pura.
 */
...
/*
 * Ordena un array de doubles y reordena un array de indices asociados.
 * Se usa para ordenar probabilidades de salida o normalizar datos.
 * Complejidad promedio: O(n log(n)) - peor caso: O(n^2).
 * Se usa particion de Lomuto como requiere el enunciado.
 */
void quicksort(double *arr, int *indices, int low, int high);

/*
 * Funcion de particion de Lomuto para QuickSort.
 * Elige el ultimo elemento como pivot.
 * Complejidad: O(n) por particion.
 */
 ...
 * Cada linea: 4 floats + 1 entero (clase).
 ...
 * Emplea QuickSort para ordenar los indices antes de la separacion.
 */
...
/*
 * Imprime metricas de evaluacion: precision por clase y accuracy global.
 */
...
/*
 * Mide el tiempo de ejecucion del Forward Propagation sobre todo el dataset
